...
Wangsamas-Firmware supports up to 4 extra motors that can be controlled by
...
class StepperDriver : public MotorDriverInterface
{
   ...
   float StepsPerUnit;
public:
    StepperDriver(float _StepsPerUnit,float speed)
    {
        StepsPerUnit = _StepsPerUnit;
        delayUS = 500000 / (speed * StepsPerUnit);
    }
    void initialize() {
    ...
    {
        position = floor(newPos * StepsPerUnit + 0.5f);
    }
    void gotoPosition(float newPos)
    {
        enable();
        int32_t target = floor(newPos * StepsPerUnit + 0.5f) - position;
        position += target;
        if(target > 0) {

...
void Extruder::selectExtruderById(uint8_t extruderId)
{
    ...
    if(Printer::isHomed() && extruder[extruderId].zOffset < Extruder::current->zOffset) { // prevent extruder from hitting bed
		Printer::offsetZ = -extruder[extruderId].zOffset * Printer::invaxisStepsPerUnit[Z_AXIS];
    ...
    Printer::currentPositionSteps[E_AXIS] = Extruder::current->extrudePosition;
		...
		Printer::axisStepsPerUnit[E_AXIS] = Extruder::current->StepsPerUnit;
		Printer::invaxisStepsPerUnit[E_AXIS] = 1.0f / Printer::axisStepsPerUnit[E_AXIS];
	#endif
...
//   max_start_speed_units_per_second[E_AXIS] = Extruder::current->maxStartFeedrate;
    ...
    Printer::maxTravelAccelerationStepsPerSquareSecond[E_AXIS] =
        Printer::maxPrintAccelerationStepsPerSquareSecond[E_AXIS] = Printer::maxAccelerationMMPerSquareSecond[E_AXIS] * Printer::axisStepsPerUnit[E_AXIS];
#if USE_ADVANCE
    Printer::maxExtruderSpeed = (ufast8_t)floor(HAL::maxExtruderTimerFrequency() / (Extruder::current->maxFeedrate * Extruder::current->StepsPerUnit));
    ...
    float fmax = ((float)HAL::maxExtruderTimerFrequency() / ((float)Printer::maxExtruderSpeed * Printer::axisStepsPerUnit[E_AXIS])); // Limit feedrate to interrupt speed
	...
	void Extruder::recomputeMixingExtruderSteps() {
		...
		for(fast8_t i = 0; i < NUM_EXTRUDER; i++) {
			...
			sum += extruder[i].StepsPerUnit * extruder[i].mixingW;
		}
		sum /= sum_w;
		Printer::currentPositionSteps[E_AXIS] =  Printer::currentPositionSteps[E_AXIS] * sum / Printer::axisStepsPerUnit[E_AXIS]; // reposition according resolution change
	    Printer::destinationSteps[E_AXIS] = Printer::currentPositionSteps[E_AXIS];
	    Printer::axisStepsPerUnit[E_AXIS] = sum;
	    Printer::invaxisStepsPerUnit[E_AXIS] = 1.0f / Printer::axisStepsPerUnit[E_AXIS];
	}
#endif
...
void Extruder::setTemperatureForExtruder(float temperatureInCelsius, uint8_t extr, bool beep, bool wait)
{
      ...
      )
    {
        ...
        do
        {
            ...
            {
                PrintLine::moveRelativeDistanceInSteps(0, 0, 0, -actExtruder->waitRetractUnits * Printer::axisStepsPerUnit[E_AXIS], actExtruder->maxFeedrate / 4, false, false);
                retracted = 1;
            }
#endif
            if((waituntil == 0 &&
...
{
    float oldFeedrate = Printer::feedrate;
    int32_t distance = static_cast<int32_t>(dist * StepsPerUnit / Printer::extrusionFactor);
...
void Extruder::retract(bool isRetract,bool isLong)
{
    ...
    int32_t zlift = static_cast<int32_t>(zLiftF * Printer::axisStepsPerUnit[Z_AXIS]);
...
#if NUM_EXTRUDER > 0
    {
        0,EXT0_X_OFFSET,EXT0_Y_OFFSET,EXT0_Z_OFFSET,EXT0_STEPS_PER_UNIT,EXT0_ENABLE_PIN,EXT0_ENABLE_ON,
...
#if NUM_EXTRUDER > 1
    ,{
        1,EXT1_X_OFFSET,EXT1_Y_OFFSET,EXT1_Z_OFFSET,EXT1_STEPS_PER_UNIT,EXT1_ENABLE_PIN,EXT1_ENABLE_ON,
...
#if NUM_EXTRUDER > 2
    ,{
        2,EXT2_X_OFFSET,EXT2_Y_OFFSET,EXT2_Z_OFFSET,EXT2_STEPS_PER_UNIT,EXT2_ENABLE_PIN,EXT2_ENABLE_ON,
...
#if NUM_EXTRUDER > 3
    ,{
        3,EXT3_X_OFFSET,EXT3_Y_OFFSET,EXT3_Z_OFFSET,EXT3_STEPS_PER_UNIT,EXT3_ENABLE_PIN,EXT3_ENABLE_ON,
...
#if NUM_EXTRUDER > 4
    ,{
        4,EXT4_X_OFFSET,EXT4_Y_OFFSET,EXT4_Z_OFFSET,EXT4_STEPS_PER_UNIT,EXT4_ENABLE_PIN,EXT4_ENABLE_ON,
...
#if NUM_EXTRUDER > 5
    ,{
        5,EXT5_X_OFFSET,EXT5_Y_OFFSET,EXT5_Z_OFFSET,EXT5_STEPS_PER_UNIT,EXT5_ENABLE_PIN,EXT5_ENABLE_ON,
